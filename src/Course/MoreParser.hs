...
hex ::
  Parser Char
hex = toChar <$> replicateA 4 (satisfy isHexDigit)
  where toChar str = case readHex str of Full x -> chr x
                                         Empty  -> chr 0
...
eof ::
  Parser ()
eof = P $ \str -> case str of Nil -> Result Nil ()
...
betweenSepbyComma ::
  ...
  -> Parser a
  -> Parser (List a)
betweenSepbyComma cha chb pa = betweenCharTok cha chb $ sepby pa commaTok
