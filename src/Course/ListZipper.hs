...
instance Functor ListZipper where
  f <$> ListZipper l x r = ListZipper (map f l) (f x) (map f r) 
...
--   let nextLZ =  ListZipper ls l (x:.r)
--   in if f l then IsZ nextLZ
--       else findLeft f nextLZ  
...
findRight f (ListZipper ls x rs) =
  case break f rs of (lAdd, target:.rRemain) ->
                        IsZ $ ListZipper (reverse lAdd ++ (x:.ls)) target rRemain
                     (_, Nil) -> IsNotZ
...
moveLeftN n lz    = if n < 0 then moveRightN (-n) lz 
                             else case moveLeft lz of IsZ nextLz -> moveLeftN (n-1) nextLz
                                                      IsNotZ     -> IsNotZ
...
moveRightN n lz = if n < 0 then moveLeftN (-n) lz
                           else case moveRight lz of IsZ nextLz -> moveRightN (n-1) nextLz
                                                     IsNotZ     -> IsNotZ
...
moveLeftN' n lz@(ListZipper ls _ _) = if n < 0 then moveRightN' (-n) lz
                                         else case moveLeftN n lz of IsZ lz' -> Right lz'
...
moveRightN' ::
  ...
  -> ListZipper a
  -> Either Int (ListZipper a)
moveRightN' n lz@(ListZipper _ _ rs ) = if n < 0 then moveLeftN' (-n) lz
                                         else case moveRightN n lz of IsZ lz' -> Right lz'
...
nth ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
nth n lz@(ListZipper ls _ _) = moveLeftN (length ls - n) lz
...
-- prop> rights (end lz) == Nil
  ...
  ListZipper a
  -> ListZipper a
end lz@(ListZipper _ _ rs) = let IsZ result = moveRightN (length rs) lz in result 
...
start ::
  ListZipper a
  -> ListZipper a
start lz@(ListZipper ls _ _) = let IsZ result = moveLeftN (length ls) lz in result
...
insertPushLeft ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushLeft x (ListZipper ls a rs) = ListZipper (a:.ls) x rs
...
insertPushRight ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushRight x (ListZipper ls a rs) = ListZipper ls x (a:.rs)
...
-- /Tip:/ Use `zipWith`
  ListZipper fls fx frs <*> ListZipper ls x rs =  ListZipper (zipWith ($) fls ls) (fx x) (zipWith ($) frs rs)
...
instance Extend ListZipper where
  ...
  f <<= lz =
    ListZipper (unfoldr (\lsz -> case moveLeft lsz of IsNotZ -> Empty 
                                                      IsZ nextLsz -> Full (f nextLsz, nextLsz)) lz)
               (f lz) 
               (unfoldr (\rsz -> case moveRight rsz of IsNotZ  -> Empty
                                                       IsZ nextRsz -> Full (f nextRsz, nextRsz)) lz)
...
toListZipperN idx xs = case fromList xs of IsNotZ -> IsNotZ
                                           IsZ lz -> nth idx lz
