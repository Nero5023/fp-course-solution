...
filter ::
  ...
  -> List a
  -> List a
filter pre = foldRight (\x acc -> if pre x then x:.acc else acc) Nil
...
seqOptional (Full x:.xs) = bindOptional (\inner -> Full $ x:.inner) $ seqOptional xs  
...
find ::
  ...
  -> Optional a
find _ Nil = Empty
find pre (x:.xs) = if pre x then Full x else find pre xs  
